...
def _supports_flattened_device_query_lens() -> bool:
    # Hopper (90): original flatten path. SM121 (family 120) already runs the
    # same per-token flattened decode_len=1 indexer logits for uniform k=5;
    # the adaptive varlen flatten is torch.repeat_interleave, not sm90 headers.
    return (
        current_platform.is_cuda()
        and has_deep_gemm()
        and (

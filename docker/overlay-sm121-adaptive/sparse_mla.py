...
class DeepseekV4SparseMLAMetadataBuilder(
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec):
        # SM120 FlashInfer DSV4 decode is per-token (naturally varlen). ALWAYS
        # is required for adaptive verification; uniform k=5 already works here.
    ...
    def _build_c128a_metadata(
        ...
        if num_decode_tokens > 0:
            # First-dim slice of the full-width buffer stays contiguous.
            # [:N, :W] of a wider buffer is not, and SM120 TVM rejects it
            # ("eidx must be contiguous"). extra_topk_length still clips
            # the logical width.
